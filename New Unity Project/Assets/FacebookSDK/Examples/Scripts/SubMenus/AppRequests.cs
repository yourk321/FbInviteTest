...
namespace Facebook.Unity.Example
{
    ...
    internal class AppRequests : MenuBase
    {
        ...
        protected override void GetGui()
        {
			...
			if (this.Button ("get invite Friends")) 
			{
				
				FB.API("/me/invitable_friends?limit=1000&fields=id,name", HttpMethod.GET, this.HandleInviteResult);				
			}
		...
		protected List<object> iviteList;
		protected void HandleInviteResult(IResult result)
		{
			if (result == null)
			{
			...
			{
				iviteList = (List<object>)resultDictionary["data"];
				if (iviteList.Count > 0)
				{
					for (int i = 0; i < iviteList.Count; i++) {

						Dictionary<string, object> dic = (Dictionary<string, object>)iviteList[i];
						this.requestTo = dic["id"].ToString();
						Debug.LogError ("requestTo=" + requestTo);
						this.requestMessage = "test_invite";
						break;
					}
				}
			}
